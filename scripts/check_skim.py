...
# if you want to overwrite already existing goodfiles
overwrite_flag = True
...
# set to None if you want to check all samples
samp = "DY_1J"
...
def check_samples(base_path, verb=False, overwrite=False, sample=None):
    # Iterate through the main sample directories
    debug_print("Starting to process the sample directories...", verb)

    # If a specific sample is provided, limit to that sample only
    sample_dirs = [sample] if sample else os.listdir(base_path)
...
check_samples(base_path, verb=verbose, overwrite=overwrite_flag, sample=samp)  # Specify sample name as needed
