...
#  usage :    
#    e.g.    
#             python check_skims.py -d SKIMS_UL2016APV_01Dec23_ParticleNet_isAPV
...
def find_unclosed_files(root_dir):
    unclosed_files = []
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            if file.endswith(".root"):
                file_path = os.path.join(root, file)
                if not check_closed(file_path):
                    unclosed_files.append(file_path)
    return unclosed_files
...
def process_files(directory):
    ...
    open("unclosedfiles.txt", 'w').close()

    logfiles = glob.glob("output_*.log")
    rootfiles = glob.glob("output_*.root")

    # check if log files contain errors
    if logfiles:
    ...
    if rootfiles:
        for rootfile in rootfiles:
            ...
            if not check_closed(root_path):
                ...
                with open("unclosedfiles.txt", 'a') as unclosed_file:
